...
const VIEW_COLUMNS = [
	KEY_NO.JOB_NO, KEY_NO.STATION_MASTER, KEY_NO.CONST_NAME, KEY_NO.JOB_TYPE_DISP, KEY_NO.JOB_NAME, KEY_NO.PLAN_START_DATE, KEY_NO.PLAN_END_DATE, 
	KEY_NO.ACTUAL_START_DATE, KEY_NO.ACTUAL_END_DATE, KEY_NO.POWER_OUT_FLAG, KEY_NO.OPERATION_FLAG, KEY_NO.JOB_DEPARTMENT, KEY_NO.NAME
]
...
const app = new Vue({
	...
	methods: {
		...
		searchJob: function () {
			...
			axios.post(PROJECT_NAME +"/api/point/job/search", data)
			.then((response) => {
				...
				this.filteredList = this.items.filter(v =>{ return v.data.jobType == 0 || v.data.jobType == 1 });
			})
			.catch((error) => {
